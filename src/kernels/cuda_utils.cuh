...
inline int compute_grid_size(int total_elements, int threads_per_block = THREADS_PER_BLOCK)
{
...
inline bool check_parity(const float* cpu_out, const float* gpu_out,
                         int count, float tol = 1e-5f)
{
    double max_err = 0.0;
    for (int i = 0; i < count; i++)
    {
        ...
        {
            fprintf(stderr,
                    "[PARITY FAIL] index %d: cpu=%f  gpu=%f  diff=%e\n",

...
Tensor Tensor::randn(const std::vector<int64_t>& shape, float mean, float stddev) {
    if (stddev < 0.0f) {
...
Tensor Tensor::row(int64_t row_index) const {
    if (shape_.size() < 2) {
        throw std::invalid_argument("row requires a tensor with at least 2 dimensions");
    }
    ...
    std::copy(data_.begin() + source_start,
              data_.begin() + source_start + row_size,
              result.data_.begin());
...
Tensor Tensor::multiply(const Tensor& other) const {
    if (shape_ != other.shape_) {
        throw std::invalid_argument("multiply requires tensors with the same shape");
    }

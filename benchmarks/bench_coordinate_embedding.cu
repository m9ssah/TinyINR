...
static float max_abs_error(const float *a, const float *b, int count) {
  float max_err = 0.0f;
...
int main() {
  ...
  for (int n = 0; n < count_N; n++) {
    for (int f = 0; f < count_F; f++) {
      ...
      float max_error =
          max_abs_error(h_cpu_out.data(), h_gpu_out.data(), output_count);

      csv.write_row("coordinate_embedding", B, N, D, F, input_count,
                    output_count, med_cpu, med_h2d, med_kernel, med_d2h,
                    med_e2e, kernel_speedup, e2e_speedup, max_error, -1);
